...
export function getDefaultChannel(options) {
	...
	if (curChannels) {
		return get(channels).find(item => get(item).projectId === options.projectId && get(item).title.toLowerCase() === (options.channelName || 'general')); // TODO: store default channel in a different way
	} else {
		return null;
	}
}
